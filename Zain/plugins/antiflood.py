...
from Zainbot.core import Config, Symbols, db
from Zainbot.functions.utility import Flood

from . import HelpMenu, custom_handler, group_only, Zainbot, on_message
...
async def setflood(client: Client, message: Message):
    ...
    try:
        ...
        return await Zainbot.error(message, str(e))

    if mode.lower() not in ["mute", "kick", "ban"]:
        return await Zainbot.error(
    ...
    if count == 0:
        return await Zainbot.delete(message, "Antiflood disabled!")

    await Zainbot.delete(
        message,
...
async def antiflood(client: Client, message: Message):
    ...
    if last_user == message.from_user.id:
        if (count + 1) >= limit:
            template = (
                "**ðŸ¤« ð– ð—‡ð—ð—‚ð–¥ð—…ð—ˆð—ˆð–½ {mode}!!** \n\n"
                "**{symbol} ð–´ð—Œð–¾ð—‹:** `{mention}`\n"
                "**{symbol} ð–³ð—‚ð—…ð—… ð–£ð–ºð—ð–¾:** `ðŸ—“ï¸„1¤7 {till_date}`\n"
            )
            Zain = await message.reply_text("Flood Detected!")
            ...
            if mode == "mute":
                ...
                try:
                    ...
                    return await Zainbot.error(
                        Zain, f"__Error in Antiflood while trying to ban!__\n{str(e)}"
                    )
